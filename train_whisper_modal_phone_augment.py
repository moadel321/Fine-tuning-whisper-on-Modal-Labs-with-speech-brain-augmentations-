...
hparams = {
    ...
    "save_folder": f"{CHECKPOINT_DIR}/whisper_small_egy_save_phone", # Modified save folder name
    "output_folder": f"{CHECKPOINT_DIR}/whisper_small_egy_output_phone", # Modified output folder name
    "language": "ar",
    "task": "transcribe",
    ...
    # Augmentation Params (Sequential Phone Channel Simulation)
    "augment": True,
    "codec_prob": 1.00,      # Probability for CodecAugment (G.722 forced)
    "drop_bit_prob": 1.00,   # Probability for DropBitResolution (8-bit target)
    "noise_prob": 1.00,  # Probability for AddNoise (Now active again)

    # These augmentations are applied sequentially in this order.
    # Probabilities above must be 1.0 if augment is True.
    "min_augmentations": 3, # Adjusted to match 3 active augmentations
    "max_augmentations": 3, # Adjusted to match 3 active augmentations

    # AddNoise Params (Now active again with milder SNR)
    "noise_snr_low": 0, # Cautious SNR range start
    "noise_snr_high": 15, # Cautious SNR range end

    # DropBitResolution Params
    # 'target_dtype': 'int8' will be set in Brain init (approximates 8-bit u-law)

    # Unused Augmentation Params (REMOVED):
    # reverb_prob, speed_prob, pitch_prob, gain_prob, drop_chunk_prob, drop_freq_prob
    # rir_assets_dir, rir_manifest_path, rir_scale_factor
    # speed_factors
    # pitch_steps_low, pitch_steps_high
    # gain_db_low, gain_db_high
    # drop_chunk_length_low, drop_chunk_length_high, drop_chunk_count_low, drop_chunk_count_high
    # drop_freq_count_low, drop_freq_count_high
    ...
    # === W&B Configuration ===
    "use_wandb": True,
    "wandb_project": "Whisper-Egyptian-Finetune-PhoneAug", # Modified project name
    "wandb_entity": None,
    "wandb_log_batch_freq": 100,
    "wandb_watch_model": True,
    "wandb_watch_freq": 100,
    "wandb_resume_id": None, # Set to None for a new run by default
}
...
from speechbrain.augment.time_domain import (
    AddNoise, DropBitResolution, DoClip # Keep only used time domain augs
)
...
# --- REMOVE Unused Wrappers (PitchShiftWrapper, GainWrapper) ---

# --- REMOVE RIRSampler (Replaced by AddReverb which is also removed for this specific task) ---
...
class WhisperFineTuneBrain(sb.Brain):
    def __init__(self, modules=None, opt_class=None, hparams=None, run_opts=None, checkpointer=None):
        ...
        if getattr(self.hparams, "augment", False):
            ...
            # --- RE-ENABLE AddNoise Initialization ---
            noise_prob = getattr(self.hparams, "noise_prob", 0.0)
            if noise_prob > 0:
                if noise_manifest_path and os.path.exists(noise_manifest_path) and os.path.getsize(noise_manifest_path) > 50: # Check manifest validity
                    try:
                        noise_adder = AddNoise(
                            csv_file=noise_manifest_path,
                            snr_low=getattr(self.hparams, "noise_snr_low", 15), # Use default 15
                            snr_high=getattr(self.hparams, "noise_snr_high", 20) # Use default 20
                        )
                        sb_augmentations.append(noise_adder)
                        initialized_augmentations.append("AddNoise")
                        logging.info(f"Successfully initialized AddNoise with SNR range: {getattr(self.hparams, 'noise_snr_low', 15)}-{getattr(self.hparams, 'noise_snr_high', 20)} dB.")
                    except Exception as e_addnoise:
                        logging.warning(f"Could not initialize AddNoise even with valid manifest: {e_addnoise}. Skipping.", exc_info=True)
                else:
                     logging.warning(f"Skipping AddNoise initialization because probability is 0 or manifest is invalid/empty ({noise_manifest_path}).")


            # --- REMOVE AddReverb Initialization ---

            # --- RE-ENABLE DoClip Initialization (Mild Settings) --- #
            clip_prob = getattr(self.hparams, "clip_prob", 0.0)
            if clip_prob > 0:
                try:
                    clipper = DoClip(
                        clip_low=getattr(self.hparams, "clip_low", 0.9),   # Use mild default 0.9
                        clip_high=getattr(self.hparams, "clip_high", 0.9), # Use mild default 0.9
                    )
                    sb_augmentations.append(clipper)
                    initialized_augmentations.append("DoClip")
                    logging.info(f"Initialized DoClip with clip range: {getattr(self.hparams, 'clip_low', 0.9)}-{getattr(self.hparams, 'clip_high', 0.9)}.")
                except Exception as e:
                    logging.warning(f"Could not initialize DoClip: {e}. Skipping.", exc_info=True)

            # --- Initialize DropBitResolution (Forcing 8-bit approx) ---
            drop_bit_prob = getattr(self.hparams, "drop_bit_prob", 0.0)
            if drop_bit_prob > 0:
                try:
                    # Initialize with target_dtype='int8' to approximate 8-bit
                    bit_dropper = DropBitResolution(target_dtype='int8')
                    sb_augmentations.append(bit_dropper)
                    initialized_augmentations.append("DropBitResolution (int8)")
                    logging.info("Initialized DropBitResolution with target_dtype='int8'.")
                except Exception as e:
            ...
            # --- Initialize Augmenter (SEQUENTIAL for Phone Simulation) ---
            if sb_augmentations:
                # --- Ensure augmentations are in the desired order --- #
                # NOTE: This relies on the initialization order above matching the desired sequence:
                # 1. CodecAugment (g722)
                # 2. DropBitResolution (int8)
                # 3. DoClip (+/- 40% peak)
                # 4. AddNoise (0-15 dB)
                # We will create a new list in the correct order based on initialized instances
                augmentation_map = {type(aug).__name__: aug for aug in sb_augmentations}
                ordered_augmentations = []
                desired_order = ["CodecAugment", "DropBitResolution", "AddNoise"] # REMOVED DoClip AGAIN
                missing_augs = []
                for aug_name in desired_order:
                    instance = augmentation_map.get(aug_name)
                    if instance:
                        ordered_augmentations.append(instance)
                    else:
                        # This check is important because initialization might fail
                        missing_augs.append(aug_name)
                        logging.warning(f"Required augmentation '{aug_name}' was not successfully initialized and will be missing from the sequential pipeline.")

                if len(ordered_augmentations) == len(desired_order):
                    try:
                        self.augmenter = Augmenter(
                            parallel_augment=False,        # Apply sequentially
                            concat_original=False,
                            min_augmentations=3,           # Apply all 3
                            max_augmentations=3,
                            shuffle_augmentations=False,   # Keep the specified order
                            augment_prob=1.0,            # Always apply the pipeline
                            augmentations=ordered_augmentations # Use the ordered list
                        )
                        aug_names = [type(aug).__name__ for aug in ordered_augmentations]
                        logging.info(f"SEQUENTIAL Augmenter initialized for phone simulation with {len(aug_names)} augmentations: {', '.join(aug_names)}")
                    except Exception as e:
                        logging.warning(f"Could not initialize SEQUENTIAL Augmenter: {e}. Augmentation disabled.", exc_info=True)
                        self.augmenter = None
                else:
                    logging.error(f"Could not initialize SEQUENTIAL Augmenter because required augmentations were missing: {missing_augs}. Augmentation disabled.")
                    self.augmenter = None
            else:
                logging.info("No augmentations were successfully initialized. Augmenter not created.")
